...
import { createInitialState, incrementMoves, incrementTime, setAnimating, updatePuzzleState, } from '../../core/gameState';
...
export default class PlayingScene {
    ...
    async init() {
        ...
        const gridBackground = new Graphics()
            ...
            .stroke();
        // 2. 타일 생성
        const { tilesContainer, tiles } = createTiles(this.state.puzzle, gridSize, cellSize, padding, (row, col) => this.handleTileClick(row, col));
        this.tiles = tiles;
        // 3. 그리드 컨테이너 생성 및 배치
        const gridContainer = new Container();
        gridContainer.addChild(gridBackground, tilesContainer);
        gridContainer.position.set(GAME_WIDTH / 2 - totalSize / 2, GAME_HEIGHT / 2 - 50 - totalSize / 2);
        // 4. 상태 표시 생성
    ...
    handleTileClick(row, col) {
        ...
        if (this.state.isAnimating || this.state.isComplete)
            return;
        // 타일 이동 가능 여부 확인
        ...
        if (result.canMove && result.emptyCell) {
            ...
            animateTileSwapUI(this.tiles[row][col].container, this.tiles[emptyCell.row][emptyCell.col].container, () => this.handleTileAnimationComplete(row, col, emptyCell), (isAnimating) => {
    ...
    handleTileAnimationComplete(row, col, emptyCell) {
