...
export default class SelectLevelScene {
  ...
  private currentDifficulty: { text: string; color: number; gridSize: number };
  ...
  private async init() {
    ...
    difficultySprites.forEach((sprite) => {
      ...
      sprite.position.set(
        GAME_WIDTH / 2,
        GAME_HEIGHT - cloudSprite.height * 0.96
      );
    ...
    difficultyText.position.set(
      GAME_WIDTH / 2,
      GAME_HEIGHT - cloudSprite.height * 0.65
    );
    ...
    const rightArrowTexture = await Assets.load(
    ...
    rightArrow.position.set(
      GAME_WIDTH - 20 - rightArrow.width / 2,
      difficultyText.y
    );
    ...
    buttonContainer.position.set(
      GAME_WIDTH / 2 - 100, // 버튼 너비의 절반을 빼서 중앙 정렬
