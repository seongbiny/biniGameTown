...
export default class SelectMemberScene {
    constructor(app, switchScene, gameState) {
        ...
        // 도형 생성 함수
        Object.defineProperty(this, "createShape", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: (x, y, texture, memberName) => {
                // 마스크용 도형 (테두리 없이 채우기만)
                ...
                const handleClick = () => {
                    ...
                    const buttonContainer = this.sceneContainer.getChildAt(2);
    ...
    async init() {
        ...
        const shape2 = this.createShape(shapeWidth + spacing, 0, danielTexture, 'daniel');
        // 두 번째 행: 2개의 도형
        const shape3 = this.createShape(0, shapeWidth + spacing, haniTexture, 'hani');
        const shape4 = this.createShape(shapeWidth + spacing, shapeWidth + spacing, haerinTexture, 'haerin');
        // 세 번째 행: 1개의 도형
        const shape5 = this.createShape(0, 2 * (shapeWidth + spacing), hyeinTexture, 'hyein');
        this.shapes = [shape1, shape2, shape3, shape4, shape5];
        // 모든 도형 요소를 shapesContainer에 추가
        shapesContainer.addChild(shape1.shapeMask, shape1.memberSprite, shape1.shape, shape2.shapeMask, shape2.memberSprite, shape2.shape, shape3.shapeMask, shape3.memberSprite, shape3.shape, shape4.shapeMask, shape4.memberSprite, shape4.shape, shape5.shapeMask, shape5.memberSprite, shape5.shape);
        // 도형들의 전체 영역 계산
        const totalWidth = 2 * shapeWidth + spacing; // 두 개의 도형과 간격
        const totalHeight = 3 * shapeWidth + 2 * spacing; // 세 행의 도형과 간격
        // 컨테이너 중앙 정렬
        shapesContainer.position.set((GAME_WIDTH - totalWidth) / 2, (GAME_HEIGHT - totalHeight) / 2 + 100 // 제목 텍스트 고려하여 약간 아래로 조정
        );
