...
// 현재 게임 상태를 저장하는 변수
let currentGameState = { status: GameStatus.MAIN };
 ...
 * @returns 업데이트된 게임 상태
 */
export const switchScene = (app, newState) => {
    // 현재 게임 상태 업데이트
    ...
    switch (newState.status) {
        ...
        case GameStatus.SELECT_LEVEL:
            currentScene = new SelectLevelScene(app, (state) => switchScene(app, state));
            break;
        case GameStatus.SELECT_ARTIST:
            currentScene = new SelectArtistScene(app, (state) => switchScene(app, state), currentGameState);
            break;
        case GameStatus.SELECT_MEMBER:
            currentScene = new SelectMemberScene(app, (state) => switchScene(app, state), currentGameState);
            break;
        case GameStatus.READY:
            currentScene = new ReadyScene(app, (state) => switchScene(app, state), currentGameState);
            break;
        case GameStatus.PLAYING:
            currentScene = new PlayingScene(app, (state) => switchScene(app, state), currentGameState);
            break;
        case GameStatus.GAME_OVER:
            currentScene = new GameOverScene(app, (state) => switchScene(app, state), currentGameState);
            break;
    }
 ...
 */
export const initializeScene = (app, initialState = { status: GameStatus.MAIN }) => {
