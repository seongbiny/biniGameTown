export const generateSolutionArray = (gridSize: number): number[] => {
 ...
 * @returns 역전 카운트
 */
export function getInversionCount(arr: number[]): number {
  let invCount = 0;
 ...
 */
function makeValidPuzzle(numbers: number[]): number[] {
 ...
 * @returns 2차원 타일 배열
 */
export function generateValidTiles(gridSize: number): number[][] {
