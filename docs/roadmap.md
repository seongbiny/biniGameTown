...
| Phase | 이름 | 상태 | 목표 |
|-------|------|------|------|
| Phase 1 | MVP — 게임 포털 기반 구축 | 🟡 진행 중 | 3개 미니게임을 한 곳에서 탐색하고 실행 |
...
**목표**: `pnpm dev` 한 번으로 웹사이트가 실행되고, 사용자가 3개의 미니게임을 탐색·실행할 수 있다.
**예상 기간**: 3~4주
**포함 게임**: Typo Trap, Flappy Bird, Bini Puzzle
...
### 1-2. `packages/types` 구현

- [ ] **[TASK-006]** `GameDifficulty` 타입 정의
  ```ts
  export type GameDifficulty = 'easy' | 'normal' | 'hard';
  ```
  - 담당 파일: `packages/types/src/game.ts`
  - 완료 기준: 다른 패키지에서 `@biniverse/types`로 import된다
- [ ] **[TASK-007]** `GameMeta` 인터페이스 정의
  ```ts
  ...
  ```
  - 담당 파일: `packages/types/src/game.ts`
  - 완료 기준: TypeScript 오류 없이 컴파일된다
- [ ] **[TASK-008]** `GameInstance`, `GameModule` 인터페이스 정의
  ```ts
  ...
  ```
  - 담당 파일: `packages/types/src/game.ts`
  - 완료 기준: `GameModule`을 구현한 게임 패키지가 타입 검사를 통과한다
- [ ] **[TASK-009]** `packages/types` 패키지 빌드 설정 (`package.json`, `tsconfig.json`)
  - 완료 기준: `pnpm --filter @biniverse/types build`가 성공한다

---

### 1-3. `packages/game-sdk` 구현

> 현재 `docs/game-sdk.md`는 postMessage 기반 프로토콜을 정의하고 있으나, PRD의 Game SDK는 `GameInstance` / `GameModule` 직접 호출 방식이다. Phase 1 MVP는 직접 호출 방식으로 구현하고, postMessage 프로토콜은 Phase 2 (랭킹 시스템)에서 활용한다.

- [ ] **[TASK-010]** `packages/game-sdk` 패키지 초기화 (`package.json`, `tsconfig.json`, `vite.config.ts`)
  - 담당 파일: `packages/game-sdk/`
  - 완료 기준: 패키지가 독립적으로 빌드된다
- [ ] **[TASK-011]** `GameRegistry` 구현 — 게임 등록 및 조회
  - 담당 파일: `packages/game-sdk/src/registry.ts`
  - 상세: `GameModule` 배열을 관리하고 `id`로 조회하는 싱글톤 레지스트리
  - 완료 기준: `getGame('typo-trap')`이 해당 `GameModule`을 반환한다
- [ ] **[TASK-012]** `GamePlayer` 로직 구현 — `init → start → destroy` 생명주기 관리
  - 담당 파일: `packages/game-sdk/src/player.ts`
  - 상세: 컨테이너 `HTMLElement`를 받아 `GameInstance`를 생성·실행·정리하는 클래스 또는 훅
  - 완료 기준: 게임 실행 후 페이지 이탈 시 `destroy()`가 호출되어 Pixi.js 리소스가 해제된다
- [ ] **[TASK-013]** `submitGameResult` / `onGameMessage` — postMessage 유틸 구현 (Phase 2 준비)
  - 담당 파일: `packages/game-sdk/src/messaging.ts`
  - 상세: 인터페이스만 정의하고 Phase 2에서 Supabase 연동 시 사용
  - 완료 기준: 타입 오류 없이 컴파일된다
...
- [ ] **[TASK-020]** 홈 화면 (`/`) 구현
  ...
  - 상세: 플랫폼 소개 문구, 주요 게임 카드 3개 노출, 게임 목록 진입 CTA 버튼
  - 완료 기준: 홈 화면이 렌더링되고 게임 목록 링크가 동작한다
- [ ] **[TASK-021]** 게임 목록 화면 (`/games`) 구현
  - 담당 파일: `apps/web/src/pages/GamesPage.tsx`
  - 상세: `GameRegistry`에서 전체 게임 목록을 읽어 `GameCard` 그리드로 렌더링
  - 완료 기준: 3개 게임 카드가 표시되고 각 카드 클릭 시 상세 화면으로 이동한다
- [ ] **[TASK-022]** 게임 상세 화면 (`/games/:id`) 구현
  - 담당 파일: `apps/web/src/pages/GameDetailPage.tsx`
  - 상세: 썸네일, 제목, 설명, 조작법, 난이도, 게임 실행 버튼 표시
  - 완료 기준: URL의 `id`에 해당하는 게임 정보가 표시되고 실행 버튼이 동작한다
- [ ] **[TASK-023]** 게임 실행 화면 (`/games/:id/play`) 구현
...
- [ ] **[TASK-024]** 게임 메타데이터 정적 데이터 파일 작성
  - 담당 파일: `apps/web/src/data/games.ts`
  - 상세: Typo Trap, Flappy Bird, Bini Puzzle 각각의 `GameMeta` 객체 정의
  - 완료 기준: 각 게임의 `id`, `title`, `description`, `categories`, `difficulty`, `thumbnail`, `controls`가 누락 없이 채워진다
- [ ] **[TASK-025]** 썸네일 이미지 에셋 준비
  - 담당 파일: `apps/web/public/thumbnails/`
  - 완료 기준: 3개 게임 썸네일이 목록/상세 화면에 표시된다

#### 1-4-5. `GamePlayer` 컴포넌트

- [ ] **[TASK-026]** `GamePlayer` React 컴포넌트 구현
...
> 각 게임 디렉토리(`games/typo-trap/` 등)는 현재 placeholder(`.gitkeep`)만 있으므로 소스 이식부터 시작한다.

#### Typo Trap

- [ ] **[TASK-027]** `games/typo-trap` 패키지 초기화 (`package.json`, `tsconfig.json`, `vite.config.ts`)
  - 완료 기준: `pnpm --filter @biniverse/typo-trap build`가 성공한다
- [ ] **[TASK-028]** 기존 Typo Trap 소스 이식
  - 담당 파일: `games/typo-trap/src/`
  - 완료 기준: 기존 게임 로직이 Pixi.js 8 기준으로 동작한다
- [ ] **[TASK-029]** `GameModule` 형태로 export
  - 담당 파일: `games/typo-trap/src/index.ts`
  - 완료 기준: `createGame()`으로 생성한 인스턴스의 `init/start/destroy`가 정상 동작한다

#### Flappy Bird

- [ ] **[TASK-030]** `games/flappy-bird` 패키지 초기화
  - 완료 기준: `pnpm --filter @biniverse/flappy-bird build`가 성공한다
- [ ] **[TASK-031]** 기존 Flappy Bird 소스 이식
  - 담당 파일: `games/flappy-bird/src/`
  - 완료 기준: 기존 게임 로직이 Pixi.js 8 기준으로 동작한다
- [ ] **[TASK-032]** `GameModule` 형태로 export
  - 담당 파일: `games/flappy-bird/src/index.ts`
  - 완료 기준: `createGame()`으로 생성한 인스턴스의 `init/start/destroy`가 정상 동작한다

#### Bini Puzzle

- [ ] **[TASK-033]** `games/bini-puzzle` 패키지 초기화
  - 완료 기준: `pnpm --filter @biniverse/bini-puzzle build`가 성공한다
- [ ] **[TASK-034]** 기존 Bini Puzzle 소스 이식
  - 담당 파일: `games/bini-puzzle/src/`
  - 완료 기준: 기존 게임 로직이 Pixi.js 8 기준으로 동작한다
- [ ] **[TASK-035]** `GameModule` 형태로 export
...
- [ ] **[TASK-036]** `README.md` 작성 — 프로젝트 소개, 실행 방법, 구조 설명
  - 담당 파일: `README.md` (루트)
  - 완료 기준: 저장소를 처음 보는 사람이 `pnpm dev`까지 따라할 수 있다
- [ ] **[TASK-037]** `docs/architecture.md` 업데이트 — Phase 1 최종 아키텍처 반영
  - 상세: 현재 문서의 postMessage/iframe 방식을 `GameModule` 직접 호출 방식으로 갱신
  - 완료 기준: 실제 구현과 문서가 일치한다
- [ ] **[TASK-038]** `docs/game-sdk.md` 업데이트 — `GameInstance` / `GameModule` 인터페이스 문서화
...
- 게임 종료 시 `submitGameResult` → Supabase 점수 저장
- 글로벌 랭킹 화면 구현 (게임별 Top 10)
- 내 기록 화면 구현
- `packages/game-sdk`의 postMessage 프로토콜 활성화
...
PRD는 게임과 웹 포털이 `GameInstance` 인터페이스를 통해 직접 통신하는 방식을 채택했다. 기존 `docs/game-sdk.md`의 postMessage/iframe 방식 대신 이 방식을 Phase 1에서 사용하는 이유는 다음과 같다.
...
postMessage 프로토콜은 Phase 2에서 Supabase 점수 저장 시 활용한다.
...
- React 19의 `use` 훅과 Concurrent 기능을 활용한 로딩 상태 처리가 가능하다
...
- [ ] 홈 화면(`/`)이 표시된다
- [ ] 게임 목록 화면(`/games`)에서 3개의 게임 카드를 볼 수 있다
- [ ] 각 게임의 상세 화면(`/games/:id`)으로 이동할 수 있다
- [ ] 각 게임의 실행 화면(`/games/:id/play`)에서 게임이 실행된다
- [ ] Typo Trap, Flappy Bird, Bini Puzzle을 웹사이트 안에서 실행할 수 있다
